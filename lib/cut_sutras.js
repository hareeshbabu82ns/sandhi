...
var sutras = {
    ...
    'just-cut-before-vowel': { // cut before mark.mark = is vowel
        del: function (mark) {
            ...
            // if (u.include(c.onlysoft, mark.fin) && mark.penult != c.virama) {
            if (u.include(c.onlysoft, mark.fin)) {
                fin = mark.fin;
    ...
    'cut-cons': { // 8.2.39    // just-cut two cons
        del: function (mark) {
            // u.log('M', u.include(c.Kay, mark.mark), mark.mark)
            var second;
            var first, hard, soft;
            if (mark.fin == c.n) {
                first = [mark.first, c.virama].join('');
            } else if (u.include(c.onlysoft, mark.mark) && u.include(c.soft, mark.beg)) {
                // u.log('HERE')
                var hard_fin = u.soft2hard(mark.fin);
                hard = u.wolast(mark.first);
                hard = [hard, hard_fin, c.virama].join('');
            } else if (u.include(c.Kay, mark.mark) && u.include(c.soft, mark.beg)) {
                // u.log('HERE')
    ...
    'cut-cons-before-a': {
        del: function (mark) {
            ...
            var second;
            if (u.include(c.onlysoft, mark.mark)) { //  here get - по сути а, а не u.include(c.soft, mark.beg)
                var fin = mark.fin;
    ...
    'cut-cons-after-a_': { // два after-before вместе было
        del: function (mark) {
            ...
            var first;
            var second;
            if (u.include(c.onlysoft, mark.mark) && u.include(c.soft, mark.beg)) {
                var fin = mark.fin;
    ...
    'just-cut-before-vowel_': { // cut before mark.mark = is vowel
        del: function (mark) {
            ...
            // if (u.include(c.onlysoft, mark.fin)) . . . etc:
    ...
    'cut-soft-after-vowel': { // tvidametezAm => tvidam-etezAm
        del: function (mark) {
            ...
            // u.log(1, mark) // योद्धव्यमस्मिन्  अस्मिन्
