...
function outer(samasa, next) {
    ...
    // // beg == 'इ' или все же u.include(c.all ex a, beg) ?
    ...
    // u.log('FIN', fin, fin == 'ो', 'NEXT', next)
    var clean = samasa;
    // три простые частые правила:
    if (fin == 'ो' && u.include(c.soft, beg)) clean = [u.wolast(samasa), c.visarga].join('');
    else if (fin == 'ा' && (u.include(c.allvowels, beg) || u.include(c.soft, beg))) clean = [samasa, c.visarga].join('');
    ...
    else if (u.include(c.consonants, fin) && u.include(c.allexa, beg)) clean = [samasa, c.visarga].join(''); // visarga dropped before simple
    // сейчас я заменяю в Гите visarga обратно на e, если расшифровка на e.
    // что некорректно, но здесь я расшифровки не знаю
    // правило на e:
    // else if (u.include(c.consonants, fin) && u.include(c.allexa, beg) && beg != 'ए') clean = [samasa, c.visarga].join('');
    // else if (u.include(c.consonants, fin) && beg == 'ए') clean = [samasa, u.liga('ए')].join(''); // e dropped - ayadi-sandhi
