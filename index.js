...
class sandhi {
    ...
    checkSecond = function (samasa, second) {
    ...
    del(samasa, second) {
        ...
        var cleanSecond = this.checkSecond(samasa, second);
        ...
        var o = this.options(samasa, cleanSecond, second);
        if (o.first == '') {
            return [{ firsts: [o.mark], seconds: [second], sutra: 'only-vowel' }];
        }
        var sutras = this.getSutras(o);
        ...
        sutras.forEach(function (sname) {
            ...
            var res = sutra.del(o);
            if (!res) return;
            var clean = this.possibleRes(res);
            if (!clean) return;
            clean.sutra = sname;
            ...
            case 'vowel':
                ...
                if (b.dirgha) {
                    ...
                    if (b.guna) {
                        ...
                        else if (b.mark_o_beg_soft) sutras.push('visarga-o');
    ...
    options = function (samasa, second) { // raw
        var beg = u.first(second);
        var spart = second;
    ...
    possibleRes = function (res) {
        var clean = { firsts: [], seconds: [] };
        res.firsts.forEach(function (first) {
            if (this.possible(first)) clean.firsts.push(first);
        }.bind(this))
        res.seconds.forEach(function (second) {
            if (this.possible(second)) clean.seconds.push(second);
        }.bind(this))
    ...
    addOptions = function (first, second) {
    ...
    simpleSum = function (first, second) {
        var beg = u.first(second);
    ...
    add(first, second) {
        ...
        var opt = this.addOptions(first, second);
        // if (!opt) log('add: NO OPT', 'f', first, 's', second);
        if (!opt) return this.simpleSum(first, second); // just-cut
        switch (opt.type) {
            case 'vowel':
                this.addVowelFilter(opt);
                break;
            case 'cons':
                this.addConsFilter(opt);
                break;
            case 'visarga':
                this.addVisargaFilter(opt);
                break;
        }
        var sutra = vowRules[opt.num] || consRules[opt.num] || visRules[opt.num] || cutRules[opt.num];
        if (!sutra) return this.simpleSum(first, second);
        var opts = sutra.add(opt);
        var sutras = opts.map(function (o) { return o.num });
        var res = opts.map(function (m) { return that.addResult(m) });
        if (res.length == 0) log('===== NO ADD RESULTS =====');
        if (debug) log('ADD=> RES', res);
        return res;
    }

    addVowelFilter = function (opt) {
        var fin = opt.fin;
        // var penult = opt.pen;
        var beg = opt.beg;
    ...
    int(form, flex, cflex, prefix, krit) {
